...
export default function HistoryPage() {
  ...
  const handleDelete = async (predictionId) => {
    try {
      await apiRequest(`/history/${predictionId}`, { method: 'DELETE', token })
      setItems((current) => current.filter((entry) => entry.id !== predictionId))
    } catch (requestError) {
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                  <span
                    className={`rounded-full px-3 py-1 text-xs font-bold ${
                      statusStyles[getStatusLabel(item)] ?? 'bg-slate-300/20 text-slate-100'
                    }`}
                  >
                    {getStatusLabel(item)}
                  </span>
                  <button
                    type="button"
                    className="rounded-full bg-emerald-200 px-4 py-2 text-xs font-bold text-emerald-950 transition hover:bg-emerald-100 sm:w-auto"
                  ...
                  <button
                    type="button"
                    className="rounded-full border border-rose-300/40 px-4 py-2 text-xs font-bold text-rose-100 transition hover:bg-rose-500/15 sm:w-auto"
